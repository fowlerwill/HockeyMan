...
function wp_bootstrap_register_sidebars() {
    ...
    register_sidebar(array(
      ...
      'description' => 'Homepage Only Widget',
      'before_widget' => '<div id="%1$s" class="widget %2$s">',
      'after_widget' => '</div>',
      'before_title' => '<!--',
      'after_title' => '-->',
 ...
 */
function willcwf_3posts($atts, $content=null){
	$getpost = get_posts( array('numberposts' => 3) );
	$return .= '<div class="row">';
	foreach ($getpost as $post) {
		
		if(!$img = catch_that_image($post->post_content)) {
      $img = "<!-- No Image -->";
      $c = strip_tags( truncate_html($post->post_content, 400) );
    }
    else
      $c = strip_tags( truncate_html($post->post_content, 200) );

		$return .= '<div class="col-sm-4"><a href="'.get_permalink($post->ID).'"><h3 class="ft-pg-blog-title">'.$post->post_title.'</h3>
    <div class="intro-image-crop">'.$img.'</div></a><p>'.$c."…</p>";
		$return .= "<br /><a href='" . get_permalink($post->ID) . "'><em>read more →</em></a></div>";
	}
	$return .= '</div>';
	return $return;
}
...
function catch_that_image($postString) {
  $first_img = '';
  ob_start();
  ob_end_clean();
  $output = preg_match_all('/<img.+src=[\'"]([^\'"]+)[\'"].*>/i', $postString, $matches);
  $first_img = '<img src="'.$matches[1][0].'" alt=""/>';
  if(empty($matches[1][0])) {
    return false;
  }
  return $first_img;
}
add_shortcode('threepost', 'willcwf_3posts');
?>